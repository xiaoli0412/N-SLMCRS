...
#
# 用法：
#   docker build -t nslmcrs/gateway:latest -t nslmcrs/gateway:v0.7.0 .
#   docker run -p 8787:8787 --env-file .env -v nslmcrs-data:/data nslmcrs/gateway:latest
#
# Rust 内核 sidecar（可选，数值密集计算加速）单独构建：
#   docker build -f Dockerfile.kernel -t nslmcrs/kernel:latest -t nslmcrs/kernel:v0.7.0 .
...
# VERSION 通过 --build-arg 注入主版本号，默认 v0.7.0；与前端 package.json 对齐。
